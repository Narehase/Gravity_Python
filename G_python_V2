...
class Gravity:
    ...
    def sets(self, name:str, m, r, acc = [0.,0.], axis = [0,0], color = [0,255,100], Move = "Y"): # 각 객체 저장
        ...
        self.Set_List.append([name, m, r, acc, axis, color, Move, 0])
    ...
    def Nuton(self):
        ...
        for A in self.Set_List:
            ...
            V = All_Acc * self.frame_t + 0

            Displacement = V*self.frame_t
        

            Date = self._Draw_(A[4],Seta,Displacement,100)

            if A[6] == "Y" or A[6] == "y":
                List_r.append([A[0],A[1],A[2],[Seta,Displacement],Date,A[5],A[6],V])
            elif A[6] == "N" or A[6] == "n":
                List_r.append([A[0],A[1],A[2],[Seta,All_Acc],A[4],A[5],A[6],V])
            else:
    ...
    def Navi(self):
        ...
        for A in self.Set_List:
            print("{} | Acc: {}".format(str(A[0])[0:6],str(A[3][1])[0:6]))


    def _Draw_(self,Axis,Seta,Acc,Lagrangu,J = 0) -> list:
...
a = Gravity(D_acc=[0,0],frame_time=1)
a.set_fild()
# for i in range(1):
